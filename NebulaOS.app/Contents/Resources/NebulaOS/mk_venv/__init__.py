...
class EnvBuilder:
    ...
    *un*available to the created environment.
    ...
    def _setup_pip(self, context):
        ...
        pip_files = [
            os.path.join(context.env_dir, f"bin/pip"),
            os.path.join(context.env_dir, f"bin/pip{major}"),
            os.path.join(context.env_dir, f"bin/pip{major}.{minor}"),
            os.path.join(context.env_dir, f"bin/pip-{major}.{minor}")
        ]

        data = r"""#!/usr/bin/env python3
# -*- coding: utf-8 -*-
import re
import sys
...
if __name__ == '__main__':
    ...
    sys.exit(main())
        ...
        for perm in [owner, group, other]:
            total = 0
            for i, char in enumerate(perm[::-1]):
                if char == "-":
                    continue
                total += 2 ** i
            sum += str(total)

        for pip_name in pip_files:
            with open(pip_name, "w") as f:
                f.write(data)
            os.chmod(pip_name, int(sum, 8))

        url = "https://github.com/pypa/pip/archive/refs/heads/main.zip"
        folder_path = "pip-main/src/pip"
        pip = os.path.join(context.env_dir, f"lib/python{major}.{minor}/site-packages/pip")
        ...
        with zipfile.ZipFile(io.BytesIO(zip_data)) as z:
            for zip_info in z.infolist():
                if zip_info.filename.startswith(folder_path + "/"):
                    zip_info.filename = zip_info.filename[len(folder_path) + 1:]  # remove parent folders
                    if zip_info.filename:  # avoid empty folder name
                        z.extract(zip_info, pip)
